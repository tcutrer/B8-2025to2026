...
class Set:
    ...
    def symmetric_difference(self, other_set):
        ...
        for element in self:
            if other_set.search(self.get_key(element)) == None:
                result.add(element)
        for element in other_set:
            if self.search(other_set.get_key(element)) == None:
                result.add(element)
        return result
    
    def intersect_all(self, other_sets):
        result = Set(self.get_key)
        for element in self:
            in_all = True
    ...
    def __eq__(self, other):
        """Compare if two sets have the same elements"""
        if not isinstance(other, Set):
            return False
        if len(self) != len(other):
            return False
        for element in self:
            if other.search(self.get_key(element)) is None:
                return False
        return True